...
test("renders the manifest with the per-app name", () => {
  ...
  assert.equal(fromHost.theme_color, "#000000");
  assert.equal(fromHost.icons[0].src, "/__grok/icon-180.png");
  assert.equal(fromHost.icons[1].src, "/icon-192.png");
  assert.equal(fromHost.icons[2].src, "/icon-512.png");
  assert.equal(fromHost.icons[3].src, "/icon-512-maskable.png");
  assert.equal(fromHost.icons[3].purpose, "maskable");
  assert.equal(fromHost.display, "standalone");

  const fromSite = JSON.parse(
    renderWebManifest("wild-race.grok.me", { title: "AulaNexo", color: "1F4D45" }),
  );
  assert.equal(fromSite.name, "AulaNexo");
  assert.equal(fromSite.theme_color, "#1F4D45");
  assert.equal(fromSite.background_color, "#1F4D45");
});
